...
void visitor(int id, struct SharedData *sharedData) {
    ...
    for (int i = 0; i < COUNT_PAINTINGS; ++i) {
        ...
        sem_wait(sharedData->exit_mutex); // Wait for permission to exit
        sem_wait(sharedData->mutex);
        --sharedData->countVisitors;
        --sharedData->countSpectators[i];
        ...
        sem_post(sharedData->mutex);
        sem_post(sharedData->exit_mutex); // Release permission to exit
    }

    sem_wait(sharedData->mutex); // Ensure mutual exclusion
    --sharedData->countVisitors; // Decrease the count of visitors
    sem_post(sharedData->mutex); // Release the mutex
    sem_post(sharedData->cond); // Ensure that if there were waiters in watchman, they can proceed
...
int main() {
    ...
    sharedData->mutex = sem_open("/gallery_mutex", O_CREAT | O_RDWR, 0666, 1);
    sharedData->cond = sem_open("/gallery_cond", O_CREAT | O_RDWR, 0666, 0);
    sharedData->exit_mutex = sem_open("/exit_mutex", O_CREAT | O_RDWR, 0666, 1);
    ...
    sem_close(sharedData->mutex);
    sem_close(sharedData->cond);
    sem_close(sharedData->exit_mutex);
    sem_unlink("/gallery_mutex");
    sem_unlink("/gallery_cond");
